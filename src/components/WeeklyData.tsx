...
const WeeklyData: React.FC = () => {
	...
	const { currentLocation, weeklyData, fetchWeeklyWeather } =
		useLocationStore()
	const [isLoaded, setIsLoaded] = useState(false)
	const [screenSize, setScreenSize] = useState(window.innerWidth)

	// deriving minTemp and maxTemp for the slider
	const baseMinTemperatures =
		(weeklyData &&
			weeklyData[0]?.daily?.map((item) => item.day.mintemp_c)) ||
		[]
	const baseMaxTemperatures =
		(weeklyData &&
			weeklyData[0]?.daily?.map((item) => item.day.maxtemp_c)) ||
		[]
		...
		<div className="flex-1 flex flex-col dark:bg-blue-950 dark:text-gray-50">
			...
			<div className="flex flex-col items-center divide-y">
				{weeklyData[0]?.daily?.map((info) => (
					<div
						key={info.date_epoch}
						className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 px-4 py-4 md:max-w-screen-md w-screen"
						onClick={() => handleDayClick(info)}
					>
						<div className="flex flex-col">
							<p className="font-bold">
								{weekdayConverter(info.date)}
							</p>
							<span className="text-xs italic text-opacity-70">
								{info.date.slice(5, info.date.length)}
							</span>
						</div>
						...
						{screenSize > 1024 && (
							<p className="place-self-center">
								{info.hour[0].chance_of_rain >
								info.hour[0].chance_of_snow
									? `${info.hour[0].chance_of_rain}%`
									: `${info.hour[0].chance_of_snow}%`}
							</p>
						)}
